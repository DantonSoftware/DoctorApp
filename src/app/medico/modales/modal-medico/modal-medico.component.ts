      ...
      this._medicoServicio.crear(medico).subscribe({
        next: (data) => {
          if(data.isExitoso)
          {
            ...
            this._compartidoServicio.mostrarAlerta('no se pudo crear el Medico', 'Error!');
          }
        },
        error: (e) => {
    ...
    else
    {
      ...
      this._medicoServicio.editar(medico).subscribe({
        next: (data) => {
          if(data.isExitoso)
          {
            this._compartidoServicio.mostrarAlerta('Medico ha sido actializado con Exito!', 'Completo'),
            this.modal.close("true");
          }
          else{
            this._compartidoServicio.mostrarAlerta('no se pudo modificar el Medico', 'Error!');
          }
        },
        error: (e) => {
          this._compartidoServicio.mostrarAlerta(e.error.mensaje, 'Error!');
